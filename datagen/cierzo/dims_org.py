...
HOLDING_JURISDICTIONS = [
    ("NL", 0.19), ("LU", 0.16), ("IE", 0.14), ("ES", 0.13), ("MT", 0.07),
    ("CY", 0.05), ("CH", 0.08), ("DE", 0.06), ("GB", 0.07), ("FR", 0.05),
]
SECTORS = ["Retail", "Hospitality", "Digital Services", "Travel", "Logistics",
           "Financial Services", "Health", "Education", "Media", "Industrial"]
...
IBAN_LENGTH = {
    "ES": 24, "FR": 27, "DE": 22, "IT": 27, "PT": 25, "GB": 22, "NL": 18, "BE": 16,
    "IE": 22, "PL": 28, "SE": 24, "DK": 18, "NO": 15, "FI": 18, "AT": 20, "CH": 21,
    "CZ": 24, "HU": 28, "RO": 24, "BG": 22, "GR": 27, "LT": 20, "MT": 31, "LU": 20,
    "CY": 28, "US": 24, "MX": 24, "BR": 29, "MA": 28, "TR": 26, "SG": 24, "JP": 24,
}
...
def dim_corporate_group(rng: np.random.Generator, n: int) -> pa.Table:
    ...
    names = [f"{pools.GROUP_PREFIX[p]} {pools.GROUP_ROOT[r]} {pools.GROUP_SUFFIX[s]}"
             for p, r, s in zip(pre, root, suf)]
    ...
    return pa.table({
        ...
        "vat_number": pa.array([f"{c}{n_:09d}" for c, n_ in
                                zip(country.astype(str), rng.integers(10**8, 10**9 - 1, n))]),
        ...
        "payout_iban": pa.array(
            [_synthetic_iban(c, int(v)) for c, v in
             zip(country.astype(str), rng.integers(10**9, 10**12, n))]),
    })


def _assign_mcc_by_volume(rng: np.random.Generator, weights: np.ndarray,
                          mcc_arr: np.ndarray, mcc_cat: dict) -> np.ndarray:
    ...
    if weights.size < len(cats):
        raise ValueError(f"{weights.size} merchants cannot cover {len(cats)} "
...
def dim_merchant(rng: np.random.Generator, n: int, n_groups: int,
    ...
    trade_names = [f"{pools.MERCHANT_PREFIX[p]} {pools.MERCHANT_ROOT[r]}"
                   for p, r in zip(pre, rt)]

    # Version-level attributes: these are what actually change over time.
    risk_tier = rng.choice(["low", "standard", "elevated", "high"], total,
                           p=[0.31, 0.47, 0.16, 0.06])
    ...
    cursor = 0
    for v in versions:
        cuts = sorted(rng.choice(np.arange(30, config.N_DAYS + 300), size=v - 1,
                                 replace=False)) if v > 1 else []
    ...
    tbl = pa.table({
        ...
        "country_code": pa.array(
            [pools.COUNTRIES[i][0] for i in m_country[natural_idx]]).dictionary_encode(),
        "risk_tier": pa.array(risk_tier).dictionary_encode(),
        "mdr_bps": pa.array(mdr_bps, pa.int16()),
        "onboarded_on": pa.array(
            [start_base - dt.timedelta(days=int(d))
             for d in rng.integers(1, 2200, total)], pa.date32()),
        "is_active": pa.array(rng.random(total) > 0.037, pa.bool_()),
        "settlement_currency": pa.array(
            [pools.COUNTRIES[i][3] for i in m_country[natural_idx]]).dictionary_encode(),
    ...
    meta = {
        "category_volume_measured": dict(sorted(measured_share.items(),
                                                key=lambda kv: -kv[1])),
...
def dim_merchant_site(rng: np.random.Generator, n_merchants: int,
                      home_country: np.ndarray, weights: np.ndarray) -> tuple:
    ...
    kind = rng.choice(pools.SITE_KINDS, total,
                      p=[0.34, 0.13, 0.13, 0.08, 0.16, 0.04, 0.03, 0.09])
    tbl = pa.table({
        "site_sk": pa.array(np.arange(1, total + 1), pa.int32()),
        "merchant_natural_id": pa.array([f"MER-{i:06d}" for i in owner]),
        "country_code": pa.array(
            [pools.COUNTRIES[i][0] for i in site_country]).dictionary_encode(),
        "site_kind": pa.array(kind).dictionary_encode(),
        "site_label": pa.array([f"{k}-{i:07d}" for k, i in zip(kind, range(1, total + 1))]),
        "is_card_present": pa.array(np.isin(kind, ["pos_terminal", "kiosk"]), pa.bool_()),
        "supports_3ds": pa.array(~np.isin(kind, ["pos_terminal", "kiosk", "call_center"]),
                                 pa.bool_()),
        "opened_on": pa.array(
            [config.START_DATE - dt.timedelta(days=int(d))
             for d in rng.integers(30, 2000, total)], pa.date32()),
    })
...
def dim_employee(rng: np.random.Generator, n: int, name_pools: dict) -> pa.Table:
    ...
    """
    levels = np.array([t[1] for t in pools.JOB_TITLES])
    titles = [t[0] for t in pools.JOB_TITLES]
    ...
    for i in range(1, n):
        if title[i] == "Chief Executive Officer":
            title[i] = rng.choice([t for t in title_by_level[int(lvl[i])]
                                   if t != "Chief Executive Officer"])
    ...
    return pa.table({
        ...
        "department": pa.array(rng.choice(
            ["Operations", "Risk", "Finance", "Sales", "Engineering", "Compliance"],
            n, p=[0.26, 0.14, 0.15, 0.19, 0.18, 0.08])).dictionary_encode(),
        "work_email": pa.array([f"{firsts[a]}.{lasts[b]}@cierzo.example".lower()
                                .replace(" ", "") for a, b in zip(fi, li)]),
        "personal_phone": pa.array(
            [f"+346{d:08d}" for d in rng.integers(0, 10**8, n)]),
        "salary_band": pa.array(salary_band).dictionary_encode(),
        "hired_on": pa.array(
            [dt.date(2026, 1, 1) - dt.timedelta(days=int(d))
             for d in rng.integers(60, 4000, n)], pa.date32()),
        "is_active": pa.array(rng.random(n) > 0.08, pa.bool_()),
    })


def bridge_merchant_account_manager(rng: np.random.Generator, n_merchants: int,
                                    employees: pa.Table) -> pa.Table:
    ...
    valid_from = [max(config.START_DATE - dt.timedelta(days=int(d)),
                      h + dt.timedelta(days=int(rng.integers(7, 90))))
                  for d, h in zip(offset, hired)]
    ...
    valid_to = [
        ...
        for vf, e in zip(valid_from, ends)]
    ...
    return pa.table({
        ...
        "role": pa.array(rng.choice(["primary", "secondary", "technical"], total,
                                    p=[0.62, 0.26, 0.12])).dictionary_encode(),
        "valid_from": pa.array(valid_from, pa.date32()),
        "valid_to": pa.array(valid_to, pa.date32()),
