...
import pyarrow.compute  # noqa: F401  (used as pa.compute inside build_day)

from . import config, facts, pools, shape
...
def build_day(day_idx: int, day: dt.date, k: int, rng, ctx) -> tuple[pa.Table, pa.Table]:
    ...
    cat_idx = np.where(dominant, tr["affinity_cat"][ci],
                       rng.choice(ctx["mi"].present, k, p=ctx["mi"].cat_p)).astype(np.int8)
    ...
    if refuse.any():
        ...
        cat_idx[refuse] = rng.choice(ctx["open_categories"], int(refuse.sum()),
                                     p=ctx["open_category_p"]).astype(np.int8)
        nat = np.where(refuse, ctx["mi"].sample(rng, cat_idx), nat).astype(np.int32)

    domestic = rng.random(k) < ctx["domestic_share"]
    nat = np.where(domestic, ctx["mi"].sample_domestic(
        rng, cat_idx, tr["country_idx"][ci], nat,
        ctx["region_of_country"][tr["country_idx"][ci]]), nat).astype(np.int32)
    ...
    site_off = np.where(found, ctx["site_idx_sorted"][pos_c],
                        ctx["site_start"][nat]
                        + (rng.random(k) * ctx["site_count"][nat]).astype(np.int64))
    ...
    qty = np.where(rng.random(total_lines) < 0.82, 1,
                   1 + rng.geometric(0.55, total_lines)).astype(np.int16)
    ...
    disc = np.where(rng.random(total_lines) < 0.09 + 0.28 * sens,
                    rng.choice([0.05, 0.10, 0.15, 0.20, 0.30, 0.50], total_lines,
                               p=[0.28, 0.26, 0.18, 0.14, 0.10, 0.04]), 0.0)

    # ---- currency, decided BEFORE the lines are priced -------------------------
    mcur = ctx["merchant_currency_idx"][nat]
    cur_idx = np.where(rng.random(k) < 0.88, mcur,
                       ctx["country_currency_idx"][tr["country_idx"][ci]]).astype(np.int8)
    ...
    dev_sk = ctx["dev_flat"][ctx["dev_start"][ci]
                             + (rng.random(k) * dcount).astype(np.int64)]
    ...
    block = np.where(travelling, rng.integers(0, ctx["n_blocks"], k),
                     np.where(on_vpn, ctx["vpn_blocks"][rng.integers(
                         0, ctx["vpn_blocks"].size, k)], home_block)).astype(np.int64)
    ...
    channel = np.where(ctx["site_is_cp"][site_sk - 1], 2,
                       np.where(rng.random(k) < 0.11, 4,
                                np.where(tr["channel_pref"][ci] == 1, 1, 0))).astype(np.int8)

    card_off = ctx["card_start"][ci] + (rng.random(k) * ctx["card_count"][ci]).astype(np.int64)

    def _expired(off):
        return ((ctx["card_exp_year"][off] < day.year) |
                ((ctx["card_exp_year"][off] == day.year) &
                 (ctx["card_exp_month"][off] < day.month)))
    ...
    rscore = facts.risk_score(rng, amount_eur=amount_eur, ip_risk=ip_risk,
                              is_cross_border=is_cross_border, device_new=device_new,
                              ring_member=ring, hour=hour, high_risk_mcc=high_risk)
    ...
    tds_requested = supports_3ds & (channel != 2) & (
        ~exempt_low_value | (rng.random(k) < 0.18))
    tds_ok = tds_requested & (rng.random(k) < 0.913)

    p_ok = facts.approval_probability(
        risk_score=rscore, amount_eur=amount_eur.astype(np.float32),
        is_cross_border=is_cross_border,
        funding=funding, three_ds_ok=tds_ok.astype(np.float32), hour=hour,
    ...
    gap = np.where(seq == 1, 0,
                   np.clip(rng.lognormal(4.4, 1.15, n), 8, 20_000)).astype(np.int64)
    group_start = np.concatenate([[0], np.cumsum(attempts)[:-1]]).astype(np.int64)
    cs = np.cumsum(gap)
    within = cs - np.repeat(cs[group_start], attempts)
    event_ts = ((day - _EPOCH).days * 86_400
                + intent_start[rep] + within).astype(np.int64)
    ...
    ingested = event_ts + np.where(late, rng.integers(86_400, 6 * 86_400, n),
                                   rng.integers(2, 900, n))
    ...
    fee, interchange, scheme_fee = _fee_model(
        amount_eur_minor[rep], ctx["merchant_mdr"][day_idx][nat][rep], funding[rep],
        inter_regional[rep], high_risk[rep])
    zero = ~row_approved
    fee[zero] = 0
    interchange[zero] = 0
    scheme_fee[zero] = 0
    ...
    batch_id = np.where(settled,
                        ctx["merchant_natural_to_batchbase"][nat][rep] * 100_000
                        + settle_day, -1).astype(np.int64)
    ...
    tbl = pa.table({
        ...
        "event_date": pa.array(np.full(n, (day - _EPOCH).days, dtype=np.int32), pa.date32()),
        ...
        "product_category": pa.array(
            np.array(CATEGORIES)[cat_idx[rep]]).dictionary_encode(),
        "channel": pa.array(np.array(facts.CHANNELS)[channel[rep]]).dictionary_encode(),
        "currency_code": pa.array(
            np.array(config.CURRENCIES)[cur_idx[rep]]).dictionary_encode(),
        ...
        "auth_status": pa.array(np.where(row_approved, "approved",
                                np.where(blocked[rep], "declined",
                                np.where(rng.random(n) < 0.006, "timeout",
                                         "declined")))).dictionary_encode(),
        "decline_reason_code": pa.array(
            np.where(row_approved, None,
                     ctx["reason_code"][reason_idx[rep]]), pa.string()).dictionary_encode(),
        "auth_code": pa.array(
            np.where(row_approved,
                     np.char.zfill(rng.integers(0, 10**6, n).astype(str), 6), None),
            pa.string()),
        ...
        "risk_decision": pa.array(np.where(blocked[rep], "block",
                                  np.where(review[rep], "review",
                                           "allow"))).dictionary_encode(),
        "three_ds_requested": pa.array(tds_requested[rep], pa.bool_()),
        "three_ds_result": pa.array(
            np.where(~tds_requested[rep], None,
                     np.where(tds_ok[rep], "authenticated", "failed")),
        ...
        "geo_country": pa.array(
            ctx["country_codes"][geo_country[rep]]).dictionary_encode(),
        ...
        "settlement_date": pa.array(
            np.where(settled, settle_day, None), pa.date32()),
    })
    ...
    if n_dup:
        ...
        cols["ingested_at"] = pa.compute.add(
            dup.column("ingested_at"),
            pa.array(rng.integers(30, 7200, n_dup), pa.duration("s")))
    ...
    lines = pa.table({
        "payment_intent_id": pa.array(
            ...
            pa.int64()),
        "line_no": pa.array(
            (np.arange(total_lines) - np.repeat(
                np.concatenate([[0], np.cumsum(sizes)[:-1]]), sizes) + 1).astype(np.int16),
            pa.int16()),
        "event_date": pa.array(np.full(total_lines, (day - _EPOCH).days, dtype=np.int32),
                               pa.date32()),
        ...
        "currency_code": pa.array(
            np.array(config.CURRENCIES)[cur_idx[line_of]]).dictionary_encode(),
    })
