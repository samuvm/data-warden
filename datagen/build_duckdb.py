...
def emit_sql(data_dir: pathlib.Path, glob_root: str) -> str:
    ...
    """
    parts = [f"-- generated from {data_dir.name}; do not edit by hand",
             "SET enable_progress_bar = false;"]
    for t in sorted(d.name for d in data_dir.iterdir() if d.is_dir()):
        parts.append(f"CREATE OR REPLACE VIEW {t} AS SELECT * FROM read_parquet("
...
def build(data_dir: pathlib.Path, db_path: pathlib.Path,
          glob_root: str | None = None) -> int:
    if db_path.exists():
        db_path.unlink()
...
def main() -> int:
    ...
    ap.add_argument("--emit-sql", type=pathlib.Path, default=None,
                    help="write a portable catalogue script instead of a database")
    ap.add_argument("--glob-root", default="/warehouse/data",
